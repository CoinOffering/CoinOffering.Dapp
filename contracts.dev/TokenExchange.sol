...
contract TokenExchange {
    ...
    */
    ...
    mapping (uint256 => SellingProposition) public sellingPropositions;

    uint public sellingPropositionsNumber;

    /* objects */
    struct SellingProposition {
    address seller;
    address tokenContractAddress;
    uint quantity;
    uint pricePerToken;
    uint created; // block.timestamp (absolute unix timestamps (seconds since 1970-01-01))
    uint validUntil; // block.timestamp (absolute unix timestamps (seconds since 1970-01-01))
    }

    /* constructor */
    function TokenExchange(){
        owner = msg.sender;
        sellingPropositionsNumber = 0;
    }
    ...
    function allowTokenContract(address _tokenContractAddress){
